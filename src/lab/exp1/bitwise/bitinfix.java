...
public  class bitinfix {
	...
	int res;
	int state;
	public bitinfix(String input, String vars){
		root = null;
...
//			System.out.print(current.val );
	...
	public void passinfix(String[] exparr,int inf){
		...
		for( i=0 ; i<inf; i++ ){
			...
			else if( s>0 && precedence(stack[s-1]) >= precedence(exparr[i]) && stack[s-1].compareTo("(")!=0 ){
				do{
					fexp[f++] = new String(stack[s-1]);
					s--;
				}while( s-1>0 &&  precedence(stack[s-1]) >= precedence(exparr[i]) );
	...
	public node posttotree(String[] fexpr,int f){
		...
		for( i = 0 ;  i < f ; i++ ){
			...
			if( Character.isDigit(cur) ||  cur=='-' ){
				...
				if(  stack[s-2].op.compareTo("g")!=0 && precedence(fexpr[i]) > precedence(stack[s-2].op)  ){
				...
				if(  stack[s-1].op.compareTo("g")!=0 &&  precedence(fexpr[i]) >= precedence(stack[s-1].op) ) {
				...
				if(state == 0 || state==5){
					...
					stack[s-1].in=1;
					if(fexpr[i].compareTo("<<")==0 ){
					   operations[res] = operations[res].concat("<center><h2><span style='color:#006600;'>"+stack[s-2].val + " </span><span style='color:#ff3300;'>&lt;&lt;" 							+ " </span><span style='color:#ff00ff;'>" +stack[s-1].val + "</span> = <span style='color:#6633ff;'>" + num + "</span></h2>\n" );
					}
					else{
					    operations[res] = operations[res].concat("<center><h2><span style='color:#006600;'>"+stack[s-2].val + " </span><span style='color:#ff3300;'>" 
							+ fexpr[i] + " </span><span style='color:#ff00ff;'>" +stack[s-1].val + "</span> = <span style='color:#6633ff;'>" + num + "</span></h2>\n" );
					}
					operations[res] = operations[res].concat("<p style='color:blue'>In Binary Representation:<br></p><b><span style='color:#006600;'>");
					if(fexpr[i].compareTo("<<") == 0 ){
						operations[res] = operations[res].concat( Integer.toBinaryString(stack[s-2].val) + " </span><span style='color:#ff3300;'>&lt;&lt; </span><span style='color:#ff00ff'>" + stack[s-1].val+"</span> = <span style='color:#6633ff'>" + Integer.toBinaryString(num)+"</span></b></center>");	
					}
					else if(fexpr[i].compareTo(">>") == 0){
						operations[res] = operations[res].concat( Integer.toBinaryString(stack[s-2].val) + " </span><span style='color:#ff3300;'>"+fexpr[i]+ " </span><span style='color:#ff00ff'>" + stack[s-1].val+"</span> = <span style='color:#6633ff'>" + Integer.toBinaryString(num)+"</span></b></center>");	
					}
					else
						operations[res] = operations[res].concat( Integer.toBinaryString(stack[s-2].val) + " </span><span style='color:#ff3300;'>"+fexpr[i]+ " </span><span style='color:#ff00ff'>" + Integer.toBinaryString(stack[s-1].val) +"</span> = <span style='color:#6633ff'>" + Integer.toBinaryString(num)+"</span></b></center>");	
					if(state !=5)
						temp1.in = 0;
					else
						temp1.in = 5;
...
//		print(stack[0],0);
	...
	public  static void main(String[] args){
		bitinfix tree = new bitinfix("a & ( b | c ) ^ d","1:3:4:6");
	}
