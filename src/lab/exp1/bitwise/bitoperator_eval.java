...
public class bitoperator_eval extends JApplet   
{
	public void init()
	{
	...
	{
		String str[] = {"a | b & c ^ d","( a | b ) & ( c ^ d ) ","a << b","a >> b"  };
		String reason[][] = {
			{	
				"Here , '&' operator has higher precedence than  '|' and '^' operators . So , '&' operator is operated first .",
				"Here , '^' operator has higher precedence than the '|'  operator . So , '^' operator is operated first .",
				"Here , '|' is the only operator .",
				"Answer ."
			},
			{
				"Here , '()' bracket operator has higher precedence than '&' operator . But as we have TWO bracket operators , so we apply the Left To Right evaluation rule amony the two brackets and thus expression inside the LEFT most bracket is operated first . ",
				"Here , '()' bracket  operator has higher precedence than  '&'  operator . So , the expression inside the bracket is evaluated first .",
				"Here , '&' is the only operator .",
				"Answer ."
			},
			{
				"Here , we have only one '<<' operator i.e bitwise LEFT SHIFT operator . So , the ( left side ) operand is shifted bitwise towards LEFT by the number of bits which is equal to ( right side ) operand .",
				"Answer ."
			},
			{
				"Here , we have only one '>>' operator i.e bitwise RIGHT SHIFT operator . So , the ( left side ) operand is shifted bitwise towards RIGHT by the number of bits which is equal to ( right side ) operand .",
				"Answer ."
			}
		};
		String val[] = {"3","2","0","1"};
		String str_data_type[] = {"Integer" };
		String str_exp_type[] = {"Bitwise","Logical", "Arithematic"   };
	...
	//	bitinfix my = null ;
		bitinfix tree = new bitinfix(str[0] , val[0]+":"+val[1]+":"+val[2]+":"+val[3]);
		int count_steps = 0 ;
		...
		/*	public void paint(Graphics g )
			...
			public void write_reasons(){
				...
				if(tree.operations[count_steps-1].compareTo("") != 0){
					System.out.println(tree.operations[count_steps-1]);
		...
		public void actionPerformed ( ActionEvent e )
		{
			...
			else if ( e.getSource() == start )
			{
				if( start_flag == true ){
					...
					tree = new bitinfix(input_str, a.getText()+":"+ b.getText()+":"+ c.getText()+":"+ d.getText());
